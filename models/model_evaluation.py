...
LABELS_PATH = PROJECT_ROOT / 'data' / 'processed' / 'labeled_features.csv'
...
def load_and_label():
    ...
    label_col = next((c for c in PREFERRED_LABEL_COLUMNS if c in df_labels.columns), None)
...
def split_data(df, y, label_col):
    ...
    feature_cols = [
        ...
        if c not in ('date', 'symbol', label_col) and pd.api.types.is_numeric_dtype(df[c])
    ]
    feature_matrix = df[feature_cols].fillna(0.0)

    n = len(df)
    train_end = int(TRAIN_FRACTION * n)
    cal_end = int(CALIBRATION_FRACTION * n)
...
def main():
    ...
    for fraction in TOP_K_FRACTIONS:
        k = max(1, int(len(y_test) * fraction))
        top_k_rows.append((fraction, k, sorted_y[:k].mean()))
    df_topk = pd.DataFrame(top_k_rows, columns=['fraction', 'k', 'precision'])
    df_topk.to_csv(VIS_DIR / 'precision_at_topk.csv', index=False)
    ...
    df_ev.to_csv(VIS_DIR / 'expected_value_vs_threshold.csv', index=False)
    ...
    df_wf.to_csv(VIS_DIR / 'walk_forward_metrics.csv', index=False)
...
def run_event_analysis(df):
    ...
    g = df[upper_symbols == sym.upper()].copy().sort_values('date').reset_index(drop=True)
    if 'adj_close' not in g.columns or len(g) <= 30:
        return

    g['ret_1d'] = g['adj_close'].pct_change(fill_method=None)
    g['vol_pre_5d'] = g['ret_1d'].rolling(EVENT_VOL_WINDOW, min_periods=EVENT_VOL_WINDOW).std().shift(1)

    forward_base = g['ret_1d'].shift(-1)
    g['vol_post_5d'] = forward_base.rolling(
    ...
    df_events.to_csv(VIS_DIR / 'pla_event_vol_pre_post.csv', index=False)
    ...
    plt.xticks(x_positions, [str(d) for d in df_events['date']], rotation=45, ha='right')
